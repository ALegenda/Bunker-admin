...
import { tagPlanSchema, enrichCardTags, type TagPlan } from '../domain/card-tags.js';
import { rowCard } from './workspace.js';
import { audit } from './audit.js';

export async function applyTagPlan(input: TagPlan, checksum: string) {
  const plan = tagPlanSchema.parse(input);
  return transaction(async (c) => {
    ...
    if (prior) {
      if (prior.manifest_sha256 !== checksum || prior.source_sha256 !== plan.sourceSha256)
        throw Error('Applied tag migration checksum mismatch');
      return { applied: false, report: prior.report };
    }
    const workspace = (await c.query('SELECT * FROM workspace WHERE id=1 FOR UPDATE')).rows[0];
    if (!workspace) throw Error('Initialize source rules before tag migration');
    ...
    const fix = <T extends PublicCard>(card: T, layer: keyof typeof report.updated): T => {
      ...
      const result = enrichCardTags(card, entry);
    ...
    for (const row of rows) {
      ...
      await c.query('UPDATE cards SET tags=$2,version=version+1,updated_at=now() WHERE id=$1', [
        row.id,
        after.attributes.tags,
      ]);
      await audit(c, null, 'source.tags', row.id, before, after);
    }
    ...
    for (const row of published) {
      ...
      await audit(c, null, 'source.tags.public', row.id, row.data, after);
    }
...
export async function enrichSourceTags() {
  ...
  const result = await applyTagPlan(plan, createHash('sha256').update(bytes).digest('hex'));
